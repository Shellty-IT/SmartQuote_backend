...
class PDFService {
    renderItemsTable(doc, items, startY, accentColor, W, L) {
        let Y = startY;
    ...
    renderAuditTrailPage(doc, offer, log) {
        doc.addPage();
        const W = 515;
        const L = 40;
        let Y = 40;
        ...
        details.push(['Adres IP', log.ipAddress], ['Klient (nabywca)', txt(offer.client.type === 'COMPANY' ? (offer.client.company || offer.client.name) : offer.client.name)]);
        if (offer.client.nip) {
        ...
        doc.font('Helvetica').fontSize(7).fillColor('#64748b')
            .text('Hash SHA-256 wygenerowany z zawartosci oferty (pozycje, ceny, wariant, waluta). ' +
            'Sluzy do weryfikacji integralnosci danych w momencie akceptacji.', L, Y + 4, { width: W });
        Y += 28;
        ...
        doc.font('Helvetica').fontSize(7).fillColor('#047857')
            .text('Niniejszy certyfikat potwierdza, ze osoba wskazana powyzej zaakceptowala oferte ' +
            ...
            'Hash SHA-256 umozliwia weryfikacje integralnosci tresci oferty w momencie akceptacji.', L + 10, Y + 18, { width: W - 20 });
        Y += 55;
        doc.moveTo(L, Y + 10).lineTo(L + W, Y + 10).stroke('#e2e8f0');
        doc.font('Helvetica').fontSize(7).fillColor('#94a3b8')
            .text('Certificate of Acceptance | SmartQuote AI | Wygenerowano: ' + dateTime(new Date()), L, Y + 15, { width: W, align: 'center' });
    }
    ...
    generateContractPDF(contract) {
        return new Promise((resolve, reject) => {
            ...
            if (contract.client.address) {
                doc.text(txt(contract.client.address), cbX + 8, ccY);
                ccY += 10;
            }
            if (contract.client.city) {
                doc.text(txt((contract.client.postalCode || '') + ' ' + (contract.client.city || '')), cbX + 8, ccY);
                ccY += 10;
            }
