...
describe('calculateItemTotals', () => {
    ...
    it('ensures gross = net + vat', () => {
        ...
        testCases.forEach((tc) => {
            const r = calculateItemTotals(tc);
...
describe('calculateOfferTotals', () => {
    ...
    it('sums mixed VAT rates', () => {
        const item23 = calculateItemTotals({
            quantity: 1,
            unitPrice: 100,
            vatRate: 23,
            discount: 0,
        });
        const item8 = calculateItemTotals({
            quantity: 1,
            unitPrice: 100,
            vatRate: 8,
            discount: 0,
        });
        const item0 = calculateItemTotals({
            quantity: 1,
            unitPrice: 100,
            vatRate: 0,
            discount: 0,
        ...
        const result = calculateOfferTotals([item23, item8, item0]);
...
describe('calculateMarginPercent', () => {
    ...
    it('returns negative for loss', () => {
        expect(calculateMarginPercent(120, 100)).toBe(-20);
    });

    it('handles 100% margin', () => {
        expect(calculateMarginPercent(0, 500)).toBe(100);
    });
});
...
describe('calculateMarkupPercent', () => {
    ...
    it('returns 0 when cost is 0', () => {
        expect(calculateMarkupPercent(0, 100)).toBe(0);
    });

    it('returns 100% for doubled price', () => {
        expect(calculateMarkupPercent(500, 1000)).toBe(100);
    });
});
...
describe('grossToNet', () => {
    ...
    it('handles fractional results', () => {
        expect(grossToNet(99.99, 23)).toBeCloseTo(81.29, 2);
    });
});
...
describe('netToGross', () => {
    ...
    it('round-trips correctly: net → gross → net', () => {
        ...
        const gross = netToGross(originalNet, 23);
        const backToNet = grossToNet(gross, 23);
